...
def get_polygon_prompt():
    ...
    AI将持续询问问题, 直到AI认为不需要进一步确认多边形的属性细节为止.
    AI不应该接连询问同一个问题, 对于模糊不确定的回答, AI可以为我生成一个默认答案或随机答案.

    AI最终输出一个json格式的多边形属性字典, 其中包含以下属性:
    body-type: 'static', 'dynamic'之一.
    shape-type: 'polygon', 'edge', 'circle'之一.
    vertices: 顶点坐标列表, 坐标点只能是整数或浮点数, 不能是字符串. 例如: [[0, 0], [1, 0], [1, 1], [0, 1]].
